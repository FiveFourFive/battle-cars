...
namespace Tile_Editor
{
    public partial class ToolsWindow : Form
    {
            ...
            for (int Ypos = 0; Ypos < MapSize.Height; Ypos++)
            {
                for (int Xpos = 0; Xpos < MapSize.Width; Xpos++)
                {
                    if (eventList[Ypos, Xpos].Type != (TileType)(-1))
                        EventsListBox.Items.Add(eventList[Ypos, Xpos]);
                }
            }
        ...
        {
            if (EventX.Value < MapWidth.Value && EventY.Value < MapHeight.Value)
            {
                CTile Event = new CTile();

                Event.Name = EventID.Text;

                Event.XYIndex = new Point((int)(EventX.Value), (int)(EventY.Value));

                Event.Type = (TileType)(((TileSize.Height) * TileSize.Width + TileSize.Width) + TileType.TYPE_EVENT);

                if (eventList[(int)EventY.Value, (int)EventX.Value].Type == (TileType)(-1))
                {
                    EventsListBox.Items.Add(Event);
                    eventList[(int)EventY.Value, (int)EventX.Value] = Event;
                }
                ClearBoxs();
                if (EventListChange != null)
                {
        ...
        private void UpdateEvent_Click(object sender, EventArgs e)
        {
            if (EventX.Value < MapWidth.Value && EventY.Value < MapHeight.Value)
            {
                ...
                {
                    CTile Event = (CTile)EventsListBox.Items[EventsListBox.SelectedIndex];
                    eventList[Event.XYIndex.Y, Event.XYIndex.X].Type = (TileType)(-1);
                    ...
                    EventsListBox.Items.Insert(EventsListBox.SelectedIndex, Event);
        ...
        private void RemoveEvent_Click(object sender, EventArgs e)
        {
            if (EventsListBox.SelectedIndex != -1)
            {
                CTile events = (CTile)EventsListBox.Items[EventsListBox.SelectedIndex];

                eventList[events.XYIndex.Y, events.XYIndex.X].Name = "NULL";
                eventList[events.XYIndex.Y, events.XYIndex.X].Type = (TileType)(-1);
        ...
        private void EventsListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (EventsListBox.SelectedIndex != -1)
            {
                ...
                EventID.Text = events.Name;

                EventX.Value = events.XYIndex.X;
                EventY.Value = events.XYIndex.Y;
            }
        }
