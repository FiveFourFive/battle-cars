...
RECT CCamera::GetRect()
{
	...
	if( CNumPlayers::GetInstance()->GetNumberOfPlayers() == 2)
	{
		...
		{
			m_nWidth = int(CGame::GetInstance()->GetScreenWidth() * 0.5f);
			m_nHeight = int(CGame::GetInstance()->GetScreenHeight());
		}
		else
		{
			m_nWidth = int(CGame::GetInstance()->GetScreenWidth());
			m_nHeight = int(CGame::GetInstance()->GetScreenHeight() * 0.5f);
		}
...
void CCamera::Update(void)
{
	...
	{
		this->camPosX = (float(map->GetMapWidth()) * map->GetPixelWidth()) - this->GetWidth();
	}
	if( (this->camPosY + this->GetHeight()) > map->GetMapHeight() * map->GetPixelHeight() )
	{
		this->camPosY = (float(map->GetMapHeight()) * map->GetPixelHeight()) - this->GetHeight();;
	}
